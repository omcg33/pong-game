...
import { IGameFieldParams, Score } from "../interfaces/gameField";

export interface IGameFieldForTwoPlayersSettings {
    width: number;
    height: number;
    colors: Record<string, Record<string, string>>
}
export class GameFieldForTwoPlayers extends AbstractGameField {   
    private _layers: Map<string, any>;
    private _settings: IGameFieldForTwoPlayersSettings;

    constructor(params: IGameFieldParams) {
        super(params);
        const { node, Canvas, width, height, zIndex } = params;
